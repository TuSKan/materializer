...
material_side_nav_tabs <- function(side_nav_tabs, icons = NULL, color = NULL, font_color = NULL){
  ...
  for(i in 1:length(side_nav_tabs)){
    ...
    material_side_nav_tabs[[i]] <-
      shiny::tags$li(
        class = "materialize-side-nav-tab",
        ...
        shiny::tags$a(
          ...
          onclick = paste0(
            "$('.materialize-side-nav-tab-content').hide();",
            "$('.materialize-side-nav-tab-content').trigger('hide');",
            "$('.materialize-side-nav-tab-content').trigger('hidden');",
            "$('.materialize-side-nav-tab').removeClass('active');",
