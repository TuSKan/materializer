...
#'
#' Build a shinymaterial text input
...
material_text <- function(inputId, label, placeholder = NULL, inline = FALSE, readonly = FALSE, help = NULL, type = "text", icon = NULL, color = NULL) {
  ...
  shiny::tags$div(
    ...
    class = paste("input-field", if (inline) "inline", paste0("textinput-", colornm) ,"shiny-material-text"),
    if (!is.null(icon)) {
      shiny::tags$i(
    ...
    includeInHead(
      "shiny-material-text.js",
      "shiny-material-text.css",
      style = paste0(
      '/* label focus color */
