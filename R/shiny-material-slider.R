...
material_slider <- function(inputId, label, min_value = 0, max_value = 100, initial_value = min_value, color = NULL) {
  ...
  shiny::tagList(
    shiny::tags$head(
      shiny::includeScript(
        system.file(
          file.path("js", "shiny-material-slider.js"),
          package = "shinymaterial"
        )
      ),
      shiny::includeCSS(
        system.file(
          file.path("css", "shiny-material-slider.css"),
          package = "shinymaterial"
        )
      ),
      shiny::tags$style(
      ...
      shiny::tags$form(
        ...
        shiny::tags$p(
          ...
          shiny::tags$input(
            type = "range",
            id = inputId,
            class = paste("shiny-material-slider", paste0("slider-", css.names(material_colormap(color)))),
            min = min_value,
            max = max_value,
            value = initial_value
          )
